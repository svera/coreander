...
type CLIInput struct {
	...
	// MinOccurrenceRatio is the minimum fraction of the most frequent phrase's (or word's) occurrence count that a phrase or single word must reach to be kept as a search/related-document keyword for EPUB documents. A value of 0 disables text ranking entirely.
	MinOccurrenceRatio float64 `env:"MIN_OCCURRENCE_RATIO" default:"0.1" name:"min-occurrence-ratio" help:"Minimum fraction of the most frequent phrase's (or word's) occurrence count that a phrase or single word must reach to be kept as a search/related-document keyword for EPUB documents. Set to 0 to disable text ranking."`
	...
	MinSimilarityScoreRatio float64 `env:"MIN_SIMILARITY_SCORE_RATIO" default:"0.2" name:"min-similarity-score-ratio" help:"Minimum fraction of the best match's score a document must reach to be considered similar enough to show in a \"similar document\" query. Higher values give fewer but more relevant results (can end up empty); lower values show more results but risk weak, coincidental matches."`
	// MaxSimilarityKeywords caps how many of a document's TextRank keywords are used, at most, to find "similar" documents. Must match index.defaultMaxSimilarityKeywords, since kong's default tag can't reference a Go constant. Set to 0 to disable the cap. Higher: more accurate similarity matching for documents with many keywords, at the cost of slower queries (Bleve evaluates one OR clause per keyword for every candidate document); a document with hundreds of keywords and no cap can make "similar document" queries slow enough to noticeably affect the whole app, and has been observed to trigger a rare bleve/zapx crash under concurrent indexing. Lower: faster, safer queries, but a document with more keywords than the cap only gets matched on its top ones (see Document.TextRankKeywords for how they're ordered).
	MaxSimilarityKeywords int `env:"MAX_SIMILARITY_KEYWORDS" default:"60" name:"max-similarity-keywords" help:"Maximum number of a document's TextRank keywords used to find \"similar\" documents. Set to 0 to disable the cap. Higher values improve matching accuracy for documents with many keywords but slow queries down; lower values are faster but only match on a document's top keywords."`
	// PreferMetadataLanguage makes TextRank trust a document's own metadata language directly instead of running full text language detection, trading detection of secondary/mixed languages for faster indexing. Defaults to true (the fast option); set to false to always run full language detection first.
	PreferMetadataLanguage bool `env:"PREFER_METADATA_LANGUAGE" default:"true" name:"prefer-metadata-language" help:"Trust a document's own metadata language (e.g. an EPUB's declared language) directly for text ranking instead of running full text language detection, trading detection of secondary/mixed languages for faster indexing. Set to false to always run full language detection first."`
