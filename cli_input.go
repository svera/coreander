...
type CLIInput struct {
	...
	// MaxTextRankWords caps how many words a document's extracted text can have before TextRank analysis is skipped for it entirely (the document still gets its Words count, just no TextRank keywords). TextRank's underlying graph grows with every word occurrence, not just distinct words, so a very long or repetitive document can use several hundred MB of RAM for a single document, single-threaded - enough to trigger the OOM killer on a small VM/container regardless of worker count. Set to 0 to disable the cap.
	MaxTextRankWords int `env:"MAX_TEXTRANK_WORDS" default:"0" name:"max-textrank-words" help:"Maximum number of words a document's text can have before TextRank analysis is skipped for it (the document still gets its word count, just no TextRank keywords). Protects against excessive RAM usage on very long or repetitive documents. Set to 0 to disable the cap. On memory-constrained systems (e.g. 512MB RAM or less), a value around 350000 is a reasonable starting point."`
