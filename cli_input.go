...
type CLIInput struct {
	...
	MaxTextRankWords int `env:"MAX_TEXTRANK_WORDS" default:"0" name:"max-textrank-words" help:"Maximum number of words of a document's text that TextRank analysis considers. If a document's text is longer, only its first max-textrank-words words are analyzed (its word count still reflects the whole document, just its TextRank keywords come from the opening portion only). Protects against excessive RAM usage on very long or repetitive documents. Set to 0 to disable the cap. On memory-constrained systems (e.g. 512MB RAM or less), a value around 350000 is a reasonable starting point."`
