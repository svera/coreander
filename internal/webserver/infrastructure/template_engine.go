...
func TemplateEngine(viewsFS fs.FS, printers i18n.Translations) (*html.Engine, error) {
	engine := html.NewFileSystem(http.FS(viewsFS), ".html")

	engine.AddFunc("t", func(lang, key string, values ...any) template.HTML {
		return template.HTML(printers.T(lang, key, values...))
	})
	...
	engine.AddFunc("language", func(lang string) template.HTML {
		...
		return template.HTML(printers.T(lang, "_language"))
	})
