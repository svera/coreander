...
|`--prefer-metadata-language`         |`PREFER_METADATA_LANGUAGE`    | Trust a document's own metadata language (e.g. an EPUB's declared language) directly for text ranking instead of running full text language detection, trading detection of secondary/mixed languages for faster indexing. Set to false to always run full language detection first. Defaults to true.
|`--max-similarity-candidates`        |`MAX_SIMILARITY_CANDIDATES`   | Maximum number of top-scoring matches a "similar document" query considers before pruning by `min-similarity-score-ratio` and paginating. Defaults to 200.
|`--min-similarity-score-ratio`       |`MIN_SIMILARITY_SCORE_RATIO`  | Minimum fraction of the best match's score a document must reach to be considered similar enough to show in a "similar document" query. Defaults to 0.2.
