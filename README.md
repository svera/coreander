...
* `--max-textrank-words`: caps how many words a document's text can have before TextRank analysis is skipped for it (the document still gets its word count, just no keywords). TextRank builds an in-memory graph that grows with every word occurrence, not just distinct words, so a very long or repetitive document can use a large amount of RAM for a single document, even without concurrent indexing — this flag protects small/low-memory setups against that. Defaults to 0 (no cap); on memory-constrained systems (e.g. 512MB RAM or less), a value around 350000 is a reasonable starting point.
...
|`--max-textrank-words`               |`MAX_TEXTRANK_WORDS`          | Maximum number of words a document's text can have before TextRank analysis is skipped for it (the document still gets its word count, just no TextRank keywords). Protects against excessive RAM usage on very long or repetitive documents. Set to 0 to disable the cap. Defaults to 0; on memory-constrained systems (e.g. 512MB RAM or less), a value around 350000 is a reasonable starting point.
